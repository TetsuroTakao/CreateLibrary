    ...
    public class HomeController : Controller
	{
		...
		public ActionResult Index()
		{
			...
			if (!string.IsNullOrEmpty(code))
			{
				...
				user.AccessList.LastOrDefault().Secret = Environment.GetEnvironmentVariable("ClientSecret");
				user.AccessList.LastOrDefault().AuthCode = code;
				user.AccessList.LastOrDefault().AADEndPoint = "v2.0";
				signIn = new MSGraph().GetToken(user);
				if (signIn)
				{
					new MSGraph().SetUserInfo(user);
					...
					var grant = "{\"password\":\"" + Environment.GetEnvironmentVariable("Password") + "\",\"username\":\"" + Environment.GetEnvironmentVariable("Account") + "\"}";
					...
					if (new MSGraph().GetToken(user))
					{
						var webLink = new MSGraph().GetLink("01N7LZHZCQ2D4PKWJ4OJAKWEOBBJWTCOWG", user);
					}
				}
			}
